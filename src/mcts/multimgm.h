...
inline double get_sim_expand_score(
    ...
    const size_t& k);
template <class Pattern, class DataGraph>
inline void OutputMultiDiscoverResult(SearchTreeNodeT<Pattern, DataGraph>* root,
...
inline void Expand(
    ...
    std::set<std::tuple<typename DataGraph::VertexType::LabelType,
                        ...
                        typename DataGraph::EdgeType::LabelType>>&
        edge_type_set,
    DataGraph& data_graph, const size_t& sigma,
    std::vector<DiscoverdGPAR<Pattern, DataGraph>>& this_round_gpar,
    const double& conf_limit, const double& PB);
template <class Pattern, class DataGraph>
inline void Sim(
    std::vector<DiscoverdGPAR<Pattern, DataGraph>>& this_round_gpar,
...
inline bool IsSearchOver(SearchTreeNodeT<Pattern, DataGraph>* search_tree_root,
                         ...
                         const size_t& d, const int& numprocs);

template <class Pattern, class DataGraph>
SearchTreeNodeT<Pattern, DataGraph>* FindNode(
    SearchTreeNodeT<Pattern, DataGraph>* search_tree_root, int id) {
...
inline void MultiMGM(
  ...
  if (myid == 0) {
    ...
    while (iter_num--) {
      ...
      std::vector<DiscoverdGPAR<Pattern, DataGraph>> this_round_gpar;
      Expand(best_node->discovergpar, vertex_label_set, edge_label_set,
             edge_type_set, data_graph, sigma, this_round_gpar, conf_limit, PB);
      std::vector<float> score(this_round_gpar.size());
      Sim(this_round_gpar, score, vertex_label_set, edge_label_set,
  ...
  if (myid == 0) {
    ...
    while (!IsSearchOver(search_tree_root, sigma, conf_limit, d, numprocs)) {
      ...
      for (int i = 1; i < numprocs; i++) {
        ...
        int* data = tmp.data();
        MPI_Send(data, number[0] * 2 + number[1] * 4 + 3, MPI_INT, i, 1,
                 MPI_COMM_WORLD);
      }
      auto mcts_end = std::chrono::system_clock::now();
      std::cout << "mcts : select time = "
                << CalTimeOrigin(mcts_begin, mcts_end) << std::endl;
      ...
      for (int i = 1; i < numprocs; i++) {
        ...
        for (int num = 0; num < gpar_size; num++) {
          ...
          MPI_Recv(data, number[0] * 2 + number[1] * 4 + 3 + 2, MPI_INT, i, 4,
                   MPI_COMM_WORLD, &status);  //点边+query+support Q R
          DiscoverdGPAR<Pattern, DataGraph> singlegpar(
              x_node_label, y_node_label, q_edge_label);
        ...
        std::cout << "pre children size = " << search_tree_root->children.size()
                  << std::endl;
        Update(search_tree_root, all_gpar, score);
        std::cout << "children size = " << search_tree_root->children.size()
                  << std::endl;
      }
      mcts_end = std::chrono::system_clock::now();
      std::cout << "mcts : update time = "
                << CalTimeOrigin(mcts_begin, mcts_end) << std::endl;
    ...
    while (true) {
      ...
      MPI_Recv(data, number[0] * 2 + number[1] * 4 + 3, MPI_INT, 0, 1,
      ...
      std::vector<DiscoverdGPAR<Pattern, DataGraph>> this_round_gpar;
      auto mcts_begin = std::chrono::system_clock::now();
      Expand(singlegpar, vertex_label_set, edge_label_set, edge_type_set,
             data_graph, sigma, this_round_gpar, conf_limit, PB);
      auto mcts_end = std::chrono::system_clock::now();
      std::cout << "mcts : expand time = "
                << CalTimeOrigin(mcts_begin, mcts_end) << std::endl;
      std::vector<float> score(this_round_gpar.size());
      mcts_begin = std::chrono::system_clock::now();
      Sim(this_round_gpar, score, vertex_label_set, edge_label_set,
      ...
      for (int num = 0; num < gpar_size; num++) {
        ...
        int* data = tmp.data();
        MPI_Send(data, number[0] * 2 + number[1] * 4 + 3 + 2, MPI_INT, 0, 4,
                 MPI_COMM_WORLD);
        tmp_score = score[num];
...
inline bool IsSearchOver(SearchTreeNodeT<Pattern, DataGraph>* search_tree_root,
  ...
  while (!node_stack.empty()) {
    ...
    all_node++;
    node_stack.pop();
    if (tmp->discovergpar.gpar_conf < conf_limit + EPS &&
        tmp->discovergpar.supp_R_size_ >= sigma && !tmp->is_expand &&
...
inline SearchTreeNodeT<Pattern, DataGraph>* SelectTreeRoad(
  ...
  best_node->is_expand = true;
  std::cout << "search tree max node depth = "
            << best_node->discovergpar.pattern.CountEdge()
            << " ;ID = " << best_node->node_id << std::endl;
  return best_node;
}
...
#pragma omp parallel for schedule(static, MAX_THREAD_NUM)
  for (int i = 0; i < this_round_gpar.size(); i++) {
    // sim expand score
    score[i] = get_sim_expand_score(this_round_gpar[i], vertex_label_set,
                                    edge_label_set, edge_type_set, data_graph,
...
inline void OutputMultiDiscoverResult(SearchTreeNodeT<Pattern, DataGraph>* root,
  ...
  for (auto gpar : result_gpar_list) {
    ...
    pattern_id++;
    std::cout << " this gpar conf = " << gpar.gpar_conf
